...
public class DentalStateREST extends RESTful {
    private final Object m_lock = new Object();

    private class GetAllDentalStatesListener implements Response.Listener<JSONArray> {
    ...
    public DentalStateREST(Context context) {
        setContext(context);
    }

    public Object getDentalState(int clinicId, int patientId) {

        return getAllDentalStates(clinicId, patientId);
    }

    public Object getAllDentalStatesForPatient(int patientId) {
        ...
        String url = String.format("%s://%s:%s/tscharts/v1/dentalstate?patient=%d", getProtocol(), getIP(), getPort(), patientId);

        AuthJSONArrayRequest request = new AuthJSONArrayRequest(url, null, new GetAllDentalStatesListener(), new ErrorListener());
    ...
    public Object getAllDentalStatesForClinic(int clinicid) {
        ...
        String url = String.format("%s://%s:%s/tscharts/v1/dentalstate?clinic=%d", getProtocol(), getIP(), getPort(), clinicid);
    ...
    public Object getAllDentalStates(int clinicid, int patientid) {
        ...
        String url = String.format("%s://%s:%s/tscharts/v1/dentalstate?clinic=%d&patient=%d", getProtocol(), getIP(), getPort(), clinicid, patientid);
    ...
    public Object createDentalState(DentalState state) {
        ...
        data = state.toJSONObject(false);

        String url = String.format("%s://%s:%s/tscharts/v1/dentalstate/", getProtocol(), getIP(), getPort());

        DentalStateREST.AuthJSONObjectRequest request = new DentalStateREST.AuthJSONObjectRequest(Request.Method.POST, url, data, new PostResponseListener(), new ErrorListener());
        request.setRetryPolicy(new DefaultRetryPolicy(getTimeoutInMillis(), getRetries(), DefaultRetryPolicy.DEFAULT_BACKOFF_MULT));
    ...
    public Object updateDentalState(DentalState state) {
        ...
        JSONObject data = state.toJSONObject(true);

        String url = String.format("%s://%s:%s/tscharts/v1/dentalstate/%d/", getProtocol(), getIP(), getPort(), state.getId());
